...
def _tool_call_matches_selector(
    ...
    if not tool_name_matches(
        ...
        return False
    conditions = recognized_conditions(selector.conditions)
    if conditions:
        delivery = read_observed_delivery(tool_call)
        # A resolved call is decided by its conditions alone. Reading the arguments
        # too would re-match a lane whose resolution legitimately reached nobody.
        if delivery.resolved:
            return delivery_satisfies(conditions, delivery)
    return all(
