...
def test_malformed_delivery_stamp_is_ignored_not_raised() -> None:
    ...
    assert delivery.resolved


def test_a_call_that_returned_nothing_structured_is_unresolved() -> None:
    assert not read_observed_delivery(make_tool_call()).resolved


def test_structured_content_without_stamps_still_counts_as_resolved() -> None:
    tool_call = make_tool_call(
        result_summary={
            HAS_STRUCTURED_CONTENT_SUMMARY_KEY: True,
...
def test_structured_content_dropped_for_size_stays_resolved() -> None:
    ...
    # Stamps we cannot read are not stamps that reached somebody.
    assert delivery.resolved
    assert delivery.stamps == []
...
def test_stamps_that_all_failed_to_parse_do_not_fall_back_to_arguments() -> None:
    assert not matched(
        bob_selector(),
        make_tool_call(
...
def test_stamps_dropped_for_size_do_not_fall_back_to_arguments() -> None:
    oversized = ToolResult(
        content=[],
        structured_content={
