...
class AgentTrajectoryOutput(BaseModel):
    ...
    # Denormalized last `message_type='final_answer'` emission (RLS-9433),
    # captured by the logging sink and persisted to `trajectories.final_answer` so
    # task-QA reads don't depend on the (S3-bound) log transcript. None when the
    # agent emitted no final answer.
...
class AgentDefn(BaseModel):
    ...
    # False -> run_agent skips the archipelago env sandbox even if the world has a
    # built platform image (agent delegates execution elsewhere, e.g. lighthouse).
    requires_environment_sandbox: bool = True
