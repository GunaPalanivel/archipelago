...
async def generate_response(
    ...
    if kwargs.get("api_base"):
        ...
        # Explicit trajectory_id param wins (caller knows a more specific
        # value); falls back to the ambient ContextVar set at the worker
        # entrypoint, so callers that don't thread the param through still
        # get a value where one's available.
        ...
        # trajectory_batch vs trajectory_single — just not previously
        # surfaced as its own tag. None for one-off (non-batch) runs.
