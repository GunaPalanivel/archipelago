...
def _count_nonempty_postgres_copy_blocks(sql_content: str) -> int:
    ...
    while index < len(lines):
        if not copy_header.match(lines[index].strip()):
            index += 1
            continue

        index += 1
        has_data = index < len(lines) and lines[index] != r"\."
        nonempty_blocks += has_data
...
def _execute_transpiled_statements(
    ...
    for index, stmt in enumerate(transpiled):
        ...
        if _execute_statement(conn, stmt, expression, mysql_fallback):
...
def _execute_statement(
    ...
    try:
        ...
        if mysql_fallback and expression is not None:
            for fallback in _mysql_create_table_fallbacks(expression):
                fallback = re.sub(
...
def _load_sql_dump_to_sqlite(sql_content: str) -> SqlDumpLoadResult:
    ...
    preprocessed_sql = _preprocess_sql_for_sqlglot(sql_content)
    try:
        ...
        for expression in expressions:
            ...
            if _execute_transpiled_statements(
                conn,
    ...
    for statement in _split_sql_statements(sql_content, dialect == "mysql"):
        ...
        parsed_statement_count += 1
        if _execute_transpiled_statements(
            conn, transpiled, parsed_expressions, parsed_candidate == "mysql"
        ):
            failed_statement_count += 1
