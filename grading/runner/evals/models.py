...
class EvalType(StrEnum):
    ...
    # A single agent grading the whole task, rather than one criterion checked
    # by a judge call. Kept distinct from LLM_JUDGE on purpose: the Obi export
    # gate derives its allowlist from LLM_JUDGE membership, so reusing that type
    # would silently ship this verifier to a customer.
    AGENTIC = "agentic"
...
class EvalIds(StrEnum):
    ...
    OUTPUT_LLM_WEIGHTED_WITH_VLM = "output_llm_weighted_with_vlm"
    ...
    # Same as above with a different Type vocabulary: Critical splits by whether the
    # prompt asked explicitly, and Nice-to-have / Subjective are dropped. Balboa v4
    # asked for it; a sibling rather than an edit, so worlds already on
    # criterion_output_verifier keep the vocabulary their verifiers were authored
    # against.
    CRITERION_OUTPUT_VERIFIER_EXPLICIT = "criterion_output_verifier_explicit"
    ...
    # DB state LLM tools judge - the baseline-free twin of db_diff_llm_tools:
    # same criteria and output fields, but judges the FINAL database state
    DB_STATE_LLM_TOOLS = "db_state_llm_tools"
    ...
    # DB Diff LLM Difficulty Weighted - db_diff_llm with a Low/Medium/High difficulty field
    # intended to be paired with the difficulty_weighted_average scoring method.
    DB_DIFF_LLM_DIFFICULTY_WEIGHTED = "db_diff_llm_difficulty_weighted"
...
class EvalImplInput(BaseModel):
    ...
    # Per-grading-run args from Studio's `grading_runs.grading_run_args`,
    # threaded via GradingRunTrajectoryMetadata. Evals parse the keys they
    # understand at their own boundary. The Sparta grading verifiers read
    # `regrade_attempt_id`. None means a plain grading run.
