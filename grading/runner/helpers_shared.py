...
# OUTPUT_LLM_MULTI_REPRESENTATION, which routes the database target to
# db_diff_llm_tools_eval. Without the entry here the helper is never collected,
# and that eval fails closed — score 0.0, "No database diff was available to
# evaluate." Its Grading Target dropdown omits the database option, so only an
# API-authored or imported verifier value reaches this path.
...
def compute_snapshot_diff_content_extensions(
    ...
    for verifier in verifiers:
        ...
        # This is checked BEFORE the scope-safe gate for two reasons: a
        # scope-safe delegating eval must not add ``.db`` to the union and pull
        # binary SQLite content through the text diff, and a delegating eval that
        # is NOT scope-safe must not short-circuit to None either.
        if (
